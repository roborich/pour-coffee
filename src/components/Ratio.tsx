...
const Ratio: FC<RatioProps> = ({value, onChange}) => {
    ...
    <h2>Ratio ({preset})</h2>
        <RadioGroup value={preset} onChange={handleSelect}>
          ...
          <FormControlLabel value={Preset.CUSTOM} control={<Radio />} label={<span>Custom 1 : <input disabled={preset !== Preset.CUSTOM} type="number" value={value} onInput={handleInput}/></span>} />
        </RadioGroup>
    </>
};
